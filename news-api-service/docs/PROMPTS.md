...
本文档记录了系统中所有与 LLM 对接的提示词定义位置及内容。
...
### 1.1 KEYWORD_ANALYSIS_PROMPT

**文件位置：** `agents/news_retrieval.py#L34-L60`

**用途：** 分析用户输入的关键词，提取核心语义信息用于新闻检索

**提示词内容：**
```
你是金融舆情语义分析专家。请分析用户输入的关键词，提取核心语义信息。
...
**文件位置：** `app/routes/pipeline.py#L206-L220`

**用途：** Pipeline 步骤中的关键词语义分析
...
### 2.1 EventClassificationAgent.SYSTEM_PROMPT

**文件位置：** `agents/event_classification.py#L32-L36`
...
### 2.2 EventClassifier.SYSTEM_PROMPT

**文件位置：** `core/event_classifier.py#L70-L110`

**用途：** 单条新闻事件分类（Few-Shot Prompt）
...
### 2.3 EventClassifier.BATCH_SYSTEM_PROMPT

**文件位置：** `core/event_classifier.py#L133-L155`

**用途：** 批量事件分类（多条新闻合并为一次 LLM 调用）
...
### 3.1 SentimentAnalysisAgent.SYSTEM_PROMPT

**文件位置：** `agents/sentiment_analysis.py#L28-L32`
...
### 3.2 SentimentEngine.SYSTEM_PROMPT

**文件位置：** `core/sentiment_engine.py#L49-L95`
...
### 3.3 SentimentEngine.BATCH_SYSTEM_PROMPT

**文件位置：** `core/sentiment_engine.py#L124-L145`
...
### 4.1 FundamentalImpactAgent.SYSTEM_PROMPT

**文件位置：** `agents/fundamental_impact.py#L30-L35`
...
### 4.2 ImpactAnalyzer.IMPACT_SYSTEM_PROMPT

**文件位置：** `core/impact_analyzer.py#L22-L52`
...
### 4.3 ImpactAnalyzer.CHAIN_SYSTEM_PROMPT

**文件位置：** `core/impact_analyzer.py#L54-L75`
...
### 4.4 ImpactAnalyzer.BACKTEST_SYSTEM_PROMPT

**文件位置：** `core/impact_analyzer.py#L77-L99`

**用途：** 历史同类事件回测对比

**提示词内容：**
```
你是金融历史事件分析专家。请根据给定的当前事件类型，回忆 A 股市场历史上类似事件的影响模式。
...
### 4.5 ImpactAnalyzer.BATCH_IMPACT_SYSTEM_PROMPT

**文件位置：** `core/impact_analyzer.py#L205-L242`

**用途：** 综合批量影响分析（多事件合并为单次 LLM 调用）
...
### 5.1 IndustryChainAgent.SYSTEM_PROMPT

**文件位置：** `agents/industry_chain.py#L31-L36`
...
### 5.2 IndustryChainAgent.CHAIN_PROMPT

**文件位置：** `agents/industry_chain.py#L38-L56`
...
### 6.1 EntityLinker.NER_SYSTEM_PROMPT

**文件位置：** `core/entity_linker.py#L26-L38`
...
### 6.2 EntityLinker.CHAIN_SYSTEM_PROMPT

**文件位置：** `core/entity_linker.py#L41-L53`

**用途：** 产业链上下游分析

**提示词内容：**
```
你是产业链分析专家。给定一个上市公司名称及其所属行业，分析其产业链上下游核心企业与行业。
返回严格 JSON：
{
  "company": "公司名称",
  "industry": "所属行业",
...
### 6.3 EntityLinker.BATCH_NER_PROMPT

**文件位置：** `core/entity_linker.py#L160-L172`
...
### 7.1 StrategyGenerationAgent.SYSTEM_PROMPT

**文件位置：** `agents/strategy_generation.py#L30-L35`
...
### 7.2 StrategyGenerationAgent.STRATEGY_PROMPT

**文件位置：** `agents/strategy_generation.py#L37-L56`
...
### 8.1 RiskControlAgent.SYSTEM_PROMPT

**文件位置：** `agents/risk_control.py#L30-L35`
...
### 8.2 RiskControlAgent.RISK_PROMPT

**文件位置：** `agents/risk_control.py#L37-L56`
...
### 9.1 ComplianceAgent.SYSTEM_PROMPT

**文件位置：** `agents/compliance.py#L52-L67`
...
### 10.1 FeedbackOptimizationAgent.SYSTEM_PROMPT

**文件位置：** `agents/feedback_optimization.py#L30-L35`
...
### 10.2 FeedbackOptimizationAgent.REVIEW_PROMPT

**文件位置：** `agents/feedback_optimization.py#L37-L57`
...
### 11.1 LLMPreprocessor.SYSTEM_PROMPT

**文件位置：** `core/preprocessor.py#L156-L170`
...
### 12.1 NewsAgent._build_system_prompt()

**文件位置：** `app/news_agent.py#L118-L182`
...
### 13.1 OrchestratorAgent.SYSTEM_PROMPT

**文件位置：** `agents/orchestrator.py#L62-L67`
...
| 分类 | 提示词数量 | 主要文件 |
|------|-----------|---------|
| 关键词分析 | 2 | news_retrieval.py, pipeline.py |
| 事件分类 | 3 | event_classification.py, event_classifier.py |
| 情绪分析 | 3 | sentiment_analysis.py, sentiment_engine.py |
| 基本面影响 | 5 | fundamental_impact.py, impact_analyzer.py |
| 产业链分析 | 2 | industry_chain.py |
| 实体识别 | 3 | entity_linker.py |
| 策略生成 | 2 | strategy_generation.py |
| 风控校验 | 2 | risk_control.py |
| 合规检查 | 1 | compliance.py |
| 反馈优化 | 2 | feedback_optimization.py |
| 数据预处理 | 1 | preprocessor.py |
| 新闻分析 | 1 | news_agent.py |
| 编排器 | 1 | orchestrator.py |
| **总计** | **28** | - |
...
1. **单一职责**：每个 Agent 的 SYSTEM_PROMPT 明确禁止做其他 Agent 的工作
2. **结构化输出**：所有提示词要求返回严格 JSON 格式
3. **Few-Shot 示例**：关键提示词包含示例以提高准确性
4. **批量优化**：支持批量处理以减少 LLM 调用次数
5. **兜底机制**：LLM 失败时有规则兜底逻辑
